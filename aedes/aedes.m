...
H.UIROITOOLS_COMP = uimenu(roi_tools_h,'Label','ROI comparisons...',...
                          'callback',@l_RoiComparisons,...
                          'separator','off',...
                          'checked','off',...
                          'enable','off');
...
H.SL_SLIDER = uicontrol('parent',H.FIG,...
                        'units','pixel',...
                        'position',[tmp(1)+5 tmp(2)+80 215 20],...
                        'style','slider',...
...
H.X_BTN = uicontrol('parent',H.FIG,...
                    'units','pixel',...
                    'position',[tmp(1)+5 tmp(2)+55 30 22],...
                    'style','togglebutton',...
                    'fontweight','bold',...
                    'string','X:',...
                    'value',1,...
...
H.VOXEL_VALUE_TX = uicontrol('parent',H.FIG,...
                             'units','pixel',...
                             'position',[tmp(1)+5 tmp(2)+5 tmp(3)/2-25 13],...
                             'style','text',...
                             'fontweight','normal',...
                             'string','Voxel value:',...
														 'horizontalalign','left',...
...
H.VOXEL_VALUE = uicontrol('parent',H.FIG,...
                             'units','pixel',...
                             'position',[tmp(1)+tmp(3)/2-15 tmp(2)+5 tmp(3)/2+10 13],...
                             'style','text',...
                             'fontweight','normal',...
                             'string','-',...
														 'horizontalalign','left',...
...
tmp=get(H.SLICE_TOOLS,'position');
fig_pos = get(H.FIG,'position');
%% Contrast/Brightness/Gamma tools frame
H.CONTRAST_TOOLS=uicontrol('parent',H.FIG,...
                           'style','frame',...
                           'units','pixel',...
                           'position',[tmp(1) tmp(2)-5-230 ...
                    tmp(3) 230],...
                           'backgroundcolor',GD.col.frame);


% Contrast/Brightness/Gamma text
tmp=get(H.CONTRAST_TOOLS,'position');
H.CONTRAST_TOOLS_TX = uicontrol('parent',H.FIG,...
                                'units','pixel',...
                                'position',[tmp(1)+5 tmp(2)+tmp(4)-18 150 13],...
                                'style','text',...
                                'horizontalalign','left',...
                                'fontweight','bold',...
                                'backgroundcolor',GD.col.frame,...
                                'string','Contrast Tools');


% Contrast slider ------------------------------
tmp=get(H.CONTRAST_TOOLS,'position');
H.CONTRAST_SLIDER = uicontrol('parent',H.FIG,...
                              'units','pixel',...
                              'position',[tmp(1)+5 tmp(2)+175 173 18],...
                              'style','slider',...
                              'min',0,...
                              'max',100,...
                              'value',100,...
                              ...
                              'backgroundcolor',GD.col.slider);
tmp=get(H.CONTRAST_SLIDER,'position');
% Contrast Edit box
H.CONTRAST_EDIT = uicontrol('parent',H.FIG,...
                            'units','pixel',...
                            'position',[tmp(1)+tmp(3)+3 tmp(2) 37 tmp(4)],...
                            'style','edit',...
                            'backgroundcolor','w',...
                            'string','',...
                            'callback',@l_SetContrastBrightnessGamma);
% Contrast text
h1 = uicontrol('parent',H.FIG,...
               'units','pixel',...
               ...
               'horizontalalign','left',...
               'backgroundcolor',GD.col.frame,...
               'string','Contrast (0% - 100%)');

% Brightness slider ---------------------------------
tmp=get(H.CONTRAST_SLIDER,'position');
tmp_gap=20;
H.BRIGHTNESS_SLIDER = uicontrol('parent',H.FIG,...
                                'units','pixel',...
                                ...
                                'backgroundcolor',GD.col.slider);
tmp=get(H.BRIGHTNESS_SLIDER,'position');
% Contrast Edit box
H.BRIGHTNESS_EDIT = uicontrol('parent',H.FIG,...
                              'units','pixel',...
                              'position',[tmp(1)+tmp(3)+3 tmp(2) 37 tmp(4)],...
                              'style','edit',...
                              'backgroundcolor','w',...
...
h1 = uicontrol('parent',H.FIG,...
               ...
               'horizontalalign','left',...
               'backgroundcolor',GD.col.frame,...
               'string','Brightness (-100 - +100)');

% Gamma slider ---------------------------------
tmp=get(H.BRIGHTNESS_SLIDER,'position');
H.GAMMA_SLIDER = uicontrol('parent',H.FIG,...
                           'units','pixel',...
                           'position',[tmp(1) tmp(2)-tmp(4)-tmp_gap tmp(3) tmp(4)],...
                           'style','slider',...
                           'min',0.01,...
                           'max',4.99,...
                           'value',1,...
                           'sliderstep',[0.002008 0.1],...
                           'backgroundcolor',GD.col.slider);
tmp=get(H.GAMMA_SLIDER,'position');
% Gamma Edit box
H.GAMMA_EDIT = uicontrol('parent',H.FIG,...
                         'units','pixel',...
                         'position',[tmp(1)+tmp(3)+3 tmp(2) 37 tmp(4)],...
                         'style','edit',...
                         'backgroundcolor','w',...
...
h1 = uicontrol('parent',H.FIG,...
               ...
               'horizontalalign','left',...
               'backgroundcolor',GD.col.frame,...
               'string','Gamma (0.01 - 4.99)');

if Dat.MatlabVersion>7.03
  ContrastSliderListener = handle.listener(H.CONTRAST_SLIDER,...
	  'ActionEvent',@l_SetContrastBrightnessGamma);
	BrightnessSliderListener = handle.listener(H.BRIGHTNESS_SLIDER,...
	  'ActionEvent',@l_SetContrastBrightnessGamma);
	GammaSliderListener = handle.listener(H.GAMMA_SLIDER,...
	  'ActionEvent',@l_SetContrastBrightnessGamma);
	setappdata(H.CONTRAST_SLIDER,'HandleListeners',...
	  [ContrastSliderListener,BrightnessSliderListener,...
	  GammaSliderListener])
else
  if feature('javafigures')
	ContrastSliderListener = handle.listener(H.CONTRAST_SLIDER,...
	...
	set(H.CONTRAST_SLIDER,'callback',@l_SetContrastBrightnessGamma)
	set(H.BRIGHTNESS_SLIDER,'callback',@l_SetContrastBrightnessGamma)
	set(H.GAMMA_SLIDER,'callback',@l_SetContrastBrightnessGamma)
  end
end
tmp=get(H.GAMMA_SLIDER,'position');

% Contrast autobalance
H.CONTRAST_AUTO = uicontrol('parent',H.FIG,...
                            'units','pixel',...
                            'position',...
                            [tmp(1) tmp(2)-tmp(4)-25 35 35],...
                            'string','',...
                            'CData',btn_cdata.cdata.contrast,...
                            'tooltip','Contrast auto-balance',...
                            'style','pushbutton',...
                            'callback',@l_SetAutoContrast);
tmp=get(H.CONTRAST_AUTO,'position');
...
tmp=get(H.CONTRAST_AUTO,'position');
% Clim Min text
H.CLIMMINTEXT = uicontrol('parent',H.FIG,...
                          'units','pixel',...
                          'position',...
                          [10 tmp(2)-25 50 15],...
                          'string','Clim Min:',...
                          ...
                          'style','text',...
                          'backgroundcolor',GD.col.frame);
tmp=get(H.CLIMMINTEXT,'position');
% Min (Clim(1))
H.CLIMMINEDIT = uicontrol('parent',H.FIG,...
                          'units','pixel',...
                          'position',...
                          [tmp(1)+tmp(3) tmp(2) 50 18],...
                          'string','',...
                          'style','edit',...
                          'backgroundcolor','w',...
...
tmp=get(H.CLIMMINTEXT,'position');
% Clim Range text
H.CLIMRANGE_TX = uicontrol('parent',H.FIG,...
                           'units','pixel',...
                           'position',...
                           [10 tmp(2)-25 50 15],...
                           'string','Clim range:',...
...
tmp=get(H.CLIMRANGE_TX,'position');
% Clim range popup
H.CLIMRANGE_POPUP = uicontrol('parent',H.FIG,...
                              'units','pixel',...
                              'position',...
                              [tmp(1)+tmp(3)+5 tmp(2)+5 155 15],...
                              'string',{'locked (global)',...
                    'locked (slice)','unlocked (min/max)','unlocked (auto-balanced)'},...
                              'horizontalalign','left',...
                              'style','popup',...
                              'enable','on',...
                              'value',1,...
...
'style','frame',...
    'units','pixel',...
    'position',[tmp(1) tmp(2)-288-5 tmp(3) 288],...%[tmp(1) tmp(2)-348-5+40 tmp(3) 308],...
    'backgroundcolor',GD.col.frame);
...
H.ROITOOLS_FRAME=uicontrol('parent',H.FIG,...
                           'style','frame',...
                           'units','pixel',...
                           'position',[tmp(1) tmp(2)-50 212 50]);%[10 259-40 212 70]);
tmp=get(H.ROITOOLS_FRAME,'position');
% $$$ H.DRAW_ROI(1)=uicontrol('parent',H.FIG,...
% $$$                         'style','radio',...
% $$$                         'units','pixel',...
% $$$                         'value',0,...
% $$$                         'position',[22 tmp(2)+28 150 18],...
% $$$                         'string','Draw/Edit ROI',...
% $$$                         'callback',['set(get(gcbo,''userdata''),''value'',0),'...
% $$$                     'set(gcbo,''value'',1)']);
% $$$ H.DRAW_ROI(2)=uicontrol('parent',H.FIG,...
% $$$                         'style','radio',...
% $$$                         'units','pixel',...
% $$$                         'value',1,...
% $$$                         'position',[22 tmp(2)+7 150 18],...
% $$$                         'string','Change View/Contrast',...
% $$$                         'callback',['set(get(gcbo,''userdata''),''value'',0),'...
% $$$                     'set(gcbo,''value'',1)']);
% $$$ set(H.DRAW_ROI,'userdata',H.DRAW_ROI)
% $$$ h3=uicontrol('parent',H.FIG,...
% $$$              'units','pixel',...
% $$$              'position',[15 tmp(2)+49 150 0.2*70],...
% $$$              'style','text',...
% $$$              'string','ROI Tools',...
% $$$              'horizontalalign','left',...
% $$$              'fontweight','normal',...
% $$$              'fontsize',8);%'backgroundcolor','r');
...
h1=uicontrol('parent',H.FIG,...
             'units','pixel',...
             'position',[tmp(1) tmp(2)+tmp(4)+2 66 14],...
             'style','text',...
             'string','View ROI(s)',...
             'horizontalalign','left',...
             'fontweight','normal',...
...
h1=uicontrol('parent',H.FIG,...
             'units','pixel',...
             'position',[tmp(1) tmp(2)-21 100 14],...
             'style','text',...
...
% $$$ tmp=get(H.INFOTEXT(1),'extent');
% $$$ H.INFOTEXT(2) = text('parent',H.IMAX1,...
% $$$                      'units','normalized',...
% $$$                      'position',[0.13 0.99],...
% $$$                      'verticalalign','top',...
% $$$                      'horizontalalign','left',...
...
%uistack(H.IMAX1,'bottom')
%uistack(H.IMAX2,'bottom')
%uistack(H.IMAX3,'bottom')
%uistack(H.AXES_UIPANEL,'bottom')
%uistack(H.SIDEBAR,'top')

% Set axes to the bottom of the uistack
fig_childs = get(H.FIG,'children');
ind=ismember(fig_childs,[H.IMAX1,H.IMAX2,H.IMAX3]');
...
% Set mouse for changing contrast
...
set([H.UITOGGLE_VIEW3D,H.UITOGGLE_VIEWY,H.UITOGGLE_VIEWZ],'state','off')
set([H.UITOGGLE_ARROW,H.UITOGGLE_PAN,...
     H.UITOGGLE_DRAWROI,H.UITOGGLE_ERASEROI,H.UITOGGLE_MOVEROI,...
     H.UITOGGLE_MOVEROI3D,H.UITOGGLE_GRID],...
    'enable','off')
...
try
  ...
  Dat.LockClim = 1; % 0 = clim unlocked
...
% Set default Contrast, Brightness, and Gamma.
% Store current valid value in the userdata...
Dat.Contrast = 100;
Dat.Brightness = 0;
Dat.Gamma = 1;
set(H.CONTRAST_EDIT,'string',num2str(Dat.Contrast),...
  'userdata',Dat.Contrast)
set(H.BRIGHTNESS_EDIT,'string',num2str(Dat.Brightness),...
  'userdata',Dat.Brightness)
set(H.GAMMA_EDIT,'string',num2str(Dat.Gamma),...
  'userdata',Dat.Gamma)
...
if Dat.isDataMixed
  ...
  set(H.X_BTN,'value',1,'fontweight','bold')
  set([H.UIVIEW_3D,H.UIVIEW_Y,H.UIVIEW_Z],'enable','off',...
                    'checked','off')
  set(H.UIVIEW_X,'enable','off','checked','on')
  set([H.UITOGGLE_VIEW3D,H.UITOGGLE_VIEWY,H.UITOGGLE_VIEWZ],'enable', ...
                    'off')
  ...
  if Dat.DataL>1
    set([H.X_BTN,H.X_EDIT],'enable','on')
  ...
  if Dat.ImageDim(Dat.DataInd,3)==1
    ...
    set([H.X_BTN,H.X_EDIT],'enable','off')
...
% Reset contrast/brightness/gamma sliders
set(H.CONTRAST_SLIDER,'value',100)
set(H.BRIGHTNESS_SLIDER,'value',0)
set(H.GAMMA_SLIDER,'value',1)
...
% $$$ if Dat.isDataMixed
% $$$   set(H.IMAX1,'xlim',[0.5 Dat.ImageDim(Dat.DataInd,2)+0.5],...
% $$$               'ylim',[0.5 Dat.ImageDim(Dat.DataInd,1)+0.5])
% $$$   update_ind = 1;
% $$$   try
...
if updateX
  slice1 = DATA{Dat.DataInd}.FTDATA(:,:,Dat.Slices(1),Dat.CurrentVol);
end
...
if updateZ
  slice3 = reshape(DATA{Dat.DataInd}.FTDATA(Dat.Slices(3),:,:,Dat.CurrentVol),...
...
if Dat.isDataMixed
  set(H.X_EDIT,'string',sprintf('%d',Dat.DataInd))
...
if Dat.isDataMixed
  l_UpdateRoiInfoText([],[])
  l_AxesPositions([],[],1,Dat.ZoomLevel)
end
...
% Refresh brightness/contrast
l_SetContrastBrightnessGamma(0,[],Clim,0)
...
if ~Dat.isOverlayRGB
  ...
  % Determine if thresholding should be done
%   if Dat.ImageOverlayTholdDirPos==1
% 	if Dat.ImageOverlayClim(1)>Dat.ImageOverlayThold
% 	  thold=round(((Dat.ImageOverlayClim(1)-Dat.ImOverlayMin)*256)/...
% 		(Dat.ImOverlayMax-Dat.ImOverlayMin));
% 	elseif Dat.ImageOverlayClim(2)<Dat.ImageOverlayThold
% 		thold=round(((Dat.ImageOverlayClim(2)-Dat.ImOverlayMin)*256)/...
% 		  (Dat.ImOverlayMax-Dat.ImOverlayMin));
% 	end
%   else
% 	if Dat.ImageOverlayClim(1)>Dat.ImageOverlayThold
...
if updateX
  if ~Dat.isOverlayRGB
    ...
    slice1_ind=Dat.ImageOverlay(:,:,Dat.Slices(1),volInd);
    ...
    %slice1 = zeros([sz(1) sz(2) 3],'single');
    slice1 = squeeze(Dat.ImageOverlay(:,:,Dat.Slices(1),:));
  end
end
...
if updateZ
  if ~Dat.isOverlayRGB
    slice3_ind=reshape(Dat.ImageOverlay(Dat.Slices(3),:,:,volInd),...
	  Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
    ...
    %slice3 = zeros([sz(1) sz(2) 3],'single');
    slice3 = squeeze(Dat.ImageOverlay(Dat.Slices(3),:,:,:));
  end
end
...
%   if updateX
%     if ~Dat.isOverlayRGB
%       %slice1_alpha = double(slice1_ind);
% 	  slice1_alpha = zeros(size(slice1_ind));
% 	  if Dat.ImageOverlayTholdDirPos==1
% 		%slice1_alpha(slice1_alpha<thold) = 0;
% 		%slice1_alpha(slice1_alpha>=thold) = alpha_val;
% 		slice1_alpha(slice1_ind>=thold) = alpha_val;
% 	  else
% 		slice1_alpha(slice1_ind<=thold) = alpha_val;
% 	  end
% 	  slice1_alpha(slice1_alpha_ind)=0;
%     else
%       slice1_alpha = zeros([size(slice1,1),size(slice1,2)]);
%       slice1_alpha(:,:) = alpha_val;
%     end
...
%   if updateY
%     if ~Dat.isOverlayRGB
%       %slice2_alpha = double(slice2_ind);
% 	  slice2_alpha = zeros(size(slice2_ind));
% 	  if Dat.ImageOverlayTholdDirPos==1
% 		%slice2_alpha(slice2_alpha<thold) = 0;
% 		%slice2_alpha(slice2_alpha>=thold) = alpha_val;
% 		slice2_alpha(slice2_ind>=thold) = alpha_val;
% 	  else
% 		slice2_alpha(slice2_ind<=thold) = alpha_val;
% 	  end
%     else
...
%   if updateZ
%     if ~Dat.isOverlayRGB
%       %slice3_alpha = double(slice3_ind);
% 	  slice3_alpha = zeros(size(slice3_ind));
% 	  if Dat.ImageOverlayTholdDirPos==1
% 		%slice3_alpha(slice3_alpha<thold) = 0;
% 		%slice3_alpha(slice3_alpha>=thold) = alpha_val;
% 		slice3_alpha(slice3_ind>=thold) = alpha_val;
% 	  else
% 		slice3_alpha(slice3_ind<=thold) = alpha_val;
% 	  end
%     else
...
if Dat.isDataMixed && length(DATA)>1
  h=warndlg('Image overlay is supported only for 3D volume data',...
...
if strcmpi(opt,'show')
  ...
  pause(0.1) % Make Matlab more stable...
  ...
  setappdata(th_slider,'ThresholSliderListener',...
                       thSliderListener)
  ...
  setappdata(alpha_slider,'ThresholSliderListener',...
                          alphaSliderListener)
...
%   str=sprintf('%s%s%s\n%s%d/%d\%s%dx%dx%dx%d\n%s%s\n%s%s\n%s%s',...
% 	file_str,DATA{Dat.DataInd}.HDR.fpath,DATA{Dat.DataInd}.HDR.fname,...
% 	slice_str,Dat.Slices(1),Dat.ImageDim(Dat.DataInd,3),...
% 	imagesize_str,Dat.ImageDim(Dat.DataInd,1),Dat.ImageDim(Dat.DataInd,2),...
% 	Dat.ImageDim(Dat.DataInd,3),Dat.ImageDim(Dat.DataInd,4),...
% 	zoom_str,zoom_txt,...
% 	rotation_str,rot_txt,...
% 	flip_str,flip_txt);
%   
%   set(H.INFOTEXT,'string',str);
  if Dat.ImageDim(Dat.DataInd,4)>1
	...
	set(H.INFOTEXT,'string',...
	  ...
	  DATA{Dat.DataInd}.HDR.fname],...
	  ['Slice:      ',...
	  num2str(Dat.Slices(1)),'/',num2str(Dat.ImageDim(Dat.DataInd,3))],...
	  ['Image Size: ',num2str(Dat.ImageDim(Dat.DataInd,1)),'x',...
	  num2str(Dat.ImageDim(Dat.DataInd,2)),'x',...
...
if strcmpi(get(H.UIVIEW_INFOTXTBACKGROUND,'checked'),'off')
  ...
  set([H.INFOTEXT,H.ROIINFOTEXT],'erasemode','xor',...
                    'backgroundcolor','none')
  set(H.UIVIEW_INFOTXTBACKGROUND,'checked','off')
...
switch opt
 case 'mouse' % ------------------------------------------
  ...
  % Get indice to image
  ind=add_input;
  update_ind=update_ind(update_ind~=ind);
  
  % Get handles to the image axes
  ax_h = get(H.IM(ind),'parent');
  ...
  if ind==1
    xx=min(max(round(cp(1)),1),Dat.ImageDim(Dat.DataInd,2));
    yy=min(max(round(cp(3)),1),Dat.ImageDim(Dat.DataInd,1));
    Dat.Slices = [old_slices(ind) xx yy];
  elseif ind==2
    xx=min(max(round(cp(1)),1),Dat.ImageDim(Dat.DataInd,3));
    yy=min(max(round(cp(3)),1),Dat.ImageDim(Dat.DataInd,1));
    Dat.Slices = [xx old_slices(ind) yy];
  else
    xx=min(max(round(cp(1)),1),Dat.ImageDim(Dat.DataInd,3));
    yy=min(max(round(cp(3)),1),Dat.ImageDim(Dat.DataInd,2));
    Dat.Slices = [xx yy old_slices(ind)];
  end
 ...
 case 'slider' % -----------------------------------------
   ...
   if get(H.V_BTN,'value')==1
     ...
     if ~Dat.isDataMixed
       ...
       Dat.Slices(3)=round(Dat.Slices(3)/Dat.ImageDim(dataind,1)*...
                           Dat.ImageDim(Dat.DataInd,1));
       ...
       if Dat.ImageDim(Dat.DataInd,1)<Dat.Slices(3) 
         Dat.Slices(3) = Dat.ImageDim(Dat.DataInd,1);
       end
 ...
 case 'editbox' % -----------------------------------------
  ...
  % Check X
  if Dat.isDataMixed
    x_valnew=min(max(x_val,1),Dat.DataL);
    Dat.DataInd = x_valnew;
    set(H.X_EDIT,'string',num2str(x_valnew))
  else
    x_valnew=min(max(x_val,1),Dat.ImageDim(Dat.DataInd,3));
    set(H.X_EDIT,'string',num2str(x_valnew))
  end
  ...
  % Check Z
  z_valnew=min(max(z_val,1),Dat.ImageDim(Dat.DataInd,1));
...
if any(Dat.LockClim==[0 2 3]) && Dat.isDataMixed
  l_RefreshClim([],[]);
end
...
if any(vtype==ind) 
  % View only X, Y, or Z
  Dat.AxView=vtype;
  set(H.IM(vtype),'visible','on')
  set(H.IM(ind(ind~=vtype)),'visible','off')
  
  % Hide crossbars from other axis if they are selected
  if get(H.SHOW_CROSSBARS,'value')
    set(H.CROSSBAR_LN(:,:),'visible','off')
    set(H.CROSSBAR_LN(vtype,:),'visible','on')
  end
...
if UseDim == 4
  tsdata = squeeze(DATA{1}.FTDATA(Dat.Slices(3),Dat.Slices(2),Dat.Slices(1),:));
elseif UseDim == 3
  tsdata = squeeze(DATA{1}.FTDATA(Dat.Slices(3),Dat.Slices(2),:,1));
elseif UseDim == 5
  % For mixed data
  tsdata = [];
  for ii=1:length(DATA)
    tsdata(ii) = DATA{ii}.FTDATA(Dat.Slices(3),Dat.Slices(2));
  end
end
...
try
if get(H.SHOW_CROSSBARS,'value') | ~isfield(H,'CROSSBAR_LN')
  % Draw crossbars
  ...
  if get(H.SHOW_CROSSBARS,'value')
    if any(Dat.AxView==[1 2 3])
      set(H.CROSSBAR_LN(Dat.AxView,:),'visible','on')
    else
      set(H.CROSSBAR_LN,'visible','on')
    end
  end
...
IntVal=DATA{Dat.DataInd}.FTDATA(Dat.Slices(3),...
                               Dat.Slices(2),...
                               Dat.Slices(1),...
                               Dat.CurrentVol);
...
if ischar(input_factor) && strcmpi(input_factor,'+')
  ...
  zoom_level=get(H.UIZOOM(ind),'userdata');
  if zoom_level==20
    return
  end
...
elseif input_factor==0
  ...
  %Dat.AxSize=0;
  zoom_level=0;
  set([H.UIPUSH_ZOOMIN,H.UIPUSH_ZOOMOUT],'enable','off')
...
set(H.UIZOOM,'checked','off')
set(findobj(H.FIG,'tag',['uizoom_' num2str(zoom_level)]),'checked','on')
...
if Dat.LockClim==0
  ...
  l_SetContrastBrightnessGamma(0,[],Dat.Clim,0)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% Modify Contrast/Brightness/Gamma
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function l_SetContrastBrightnessGamma(h,evd,clim_in,wbm)
try
if nargin==4
  if ~isempty(clim_in) & wbm==0
    % Calculate contrast value and to two decimals
    Dat.Contrast=(diff(clim_in)/diff(Dat.OrigClim(Dat.DataInd,:)))*100;
    %Dat.Contrast = fix(Dat.Contrast*100)/100;
    
    % Calculate Brightness
    Dat.Brightness = ((diff(clim_in)/2-(diff(Dat.OrigClim(Dat.DataInd,:))/2))./(diff(Dat.OrigClim(Dat.DataInd,:))/2))*100;
    %Dat.Brightness = fix(Dat.Brightness*100)/100;
    
    % Make sure that contrast/brightness values are valid
    Dat.Contrast = max(min(Dat.Contrast,100),0);
    Dat.Brightness = max(min(Dat.Brightness,100),-100);
    
    % Set slider values
    set(H.CONTRAST_SLIDER,'value',Dat.Contrast)
    set(H.BRIGHTNESS_SLIDER,'value',Dat.Brightness)
    ...
    % Use "tol" pixels to represent 1% shift in contrast
    % and brightness...
    tolx = 4; % Set 1% tolerance
    toly = 2;
    
    
    shift_contrast=xy(1)/tolx;
    shift_brightness=xy(2)/toly;
    Dat.Contrast = Dat.Contrast-shift_contrast;
    Dat.Brightness = Dat.Brightness+shift_brightness;
    Dat.Contrast = fix(Dat.Contrast*100)/100;
    Dat.Brightness = fix(Dat.Brightness*100)/100;
...
% If called from Contrast Slider ---------------
if h==H.CONTRAST_SLIDER
  val = get(h,'value');
  
  % Round value to one decimal
  Dat.Contrast = fix(val*100)/100;
  
% If called from Brightness Slider ----------------
elseif h==H.BRIGHTNESS_SLIDER
  val = get(h,'value');
  
  % Round value to one decimal
  Dat.Brightness = fix(val*100)/100;
  
  

% If called from Gamma Slider ---------------------
elseif h==H.GAMMA_SLIDER
  val = get(h,'value');
  
  % Round value to two decimals
  %val=round(val*100)/100
  Dat.Gamma = fix(round(val*100))/100;
  
  set(H.GAMMA_SLIDER,'value',Dat.Gamma)
  set(H.GAMMA_EDIT,'string',sprintf('%.2f',Dat.Gamma))
  set(H.FIG,'colormap',Dat.ColMap.^Dat.Gamma);
  return

% If called from Contrast Editbox -------------------
elseif h==H.CONTRAST_EDIT
...
% If called from Brightness Editbox ----------------
elseif h==H.BRIGHTNESS_EDIT
...
% If called from Gamma Editbox ---------------------
elseif h==H.GAMMA_EDIT
  
  val=str2num(get(h,'string'));
  % Check input values
  if ~isreal(val) || isempty(val) || val<0.01 || val>4.99
    set(h,'string',num2str(get(h,'userdata')))
  else
    % Round value to two decimals
    Dat.Gamma = fix(val*100)/100;
  end
  set(H.GAMMA_SLIDER,'value',Dat.Gamma)
  set(H.GAMMA_EDIT,'string',sprintf('%.2f',Dat.Gamma))
  set(H.FIG,'colormap',Dat.ColMap.^Dat.Gamma);
...
set(H.CONTRAST_EDIT,'string',sprintf('%.2f',Dat.Contrast))
set(H.BRIGHTNESS_EDIT,'string',sprintf('%.2f',Dat.Brightness))
%set(H.GAMMA_EDIT,'string',sprintf('%.2f',Dat.Gamma))

% Set Contrast
% cont = round(diff(Dat.OrigClim(Dat.DataInd,:))/2);
% Clim(1) = round(Dat.OrigClim(Dat.DataInd,1)+((100-Dat.Contrast)/100)*cont);
% Clim(2) = round(Dat.OrigClim(Dat.DataInd,2)-((100-Dat.Contrast)/100)*cont);
cont = diff(Dat.OrigClim(Dat.DataInd,:))/2;
Clim(1) = Dat.OrigClim(Dat.DataInd,1)+((100-Dat.Contrast)/100)*cont;
Clim(2) = Dat.OrigClim(Dat.DataInd,2)-((100-Dat.Contrast)/100)*cont;


% Set Brightness
% Clim = Clim+round((diff(Dat.OrigClim(Dat.DataInd,:))/2)*(Dat.Brightness/100));
Clim = Clim+(diff(Dat.OrigClim(Dat.DataInd,:))/2)*(Dat.Brightness/100);
Dat.Clim = Clim;
...
end % function l_SetContrastBrightnessGamma(h,

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% Auto-contrast -- set contrast automatically
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function l_SetAutoContrast(h,evd)
try
Clim = zeros(1,2);
...
end % function l_SetAutoContrast(h,
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function l_ResetContrast(h,evd)
try
% Look for a quick return
...
% Set Contrast
l_SetContrastBrightnessGamma(0,[])

catch
  aedes_errordump(lasterror);
end
end % function l_ResetContrast(h,
...
if ax_ind==1
  ...
  %z=Dat.Slices(1);
  %new_voxels = [x y z vol];
  I = ROI(roi_ind).voxels{Dat.DataInd}(:,:,Dat.Slices(1),Dat.CurrentVol);
elseif ax_ind==2
  x=min(max(x,1),Dat.ImageDim(Dat.DataInd,1));
  y=min(max(y,1),Dat.ImageDim(Dat.DataInd,3));
  %z=Dat.Slices(2);
  ...
  %z=Dat.Slices(3);
  %new_voxels = [z x y vol];
  I = reshape(ROI(roi_ind).voxels{Dat.DataInd}(Dat.Slices(3),:,:,Dat.CurrentVol),...
              Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
end
...
% $$$         % Try to determine which ROI is being moved
% $$$         if ~ROI(roi_ind).voxels{Dat.DataInd}(x,y,Dat.Slices(1),vol)
% $$$           %% Check if other ROIs should be moved
% $$$           new_roi_ind = [];
% $$$           for ii=1:length(ROI)
% $$$             if ROI(ii).voxels{Dat.DataInd}(x,y,Dat.Slices(1),vol)
% $$$               new_roi_ind = ii;
% $$$             end
% $$$           end
...
% $$$         end
    ...
    elseif ax_ind==2
      ...
      Dat.RoiMoveOrigSlice = reshape(ROI(roi_ind).voxels{Dat.DataInd}(Dat.Slices(3),:,:,vol),...
                                     Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
      [I,J]=find(Dat.RoiMoveOrigSlice);
      Dat.RoiMoveLimits = [-min(J)+1,Dat.ImageDim(Dat.DataInd,3)-max(J);...
      ...
      for ii=1:length(ROI)
        ...
        tmp_roi(ii).color = ROI(ii).color;
			...
			if Save3Dtemplate
				...
				tmp_data.FTDATA = DATA{1}.FTDATA(:,:,Dat.Slices(1),Dat.CurrentVol);
				tmp_data.HDR =  DATA{1}.HDR;
			end
			for ii=1:length(ROI)
				if Save3Dtemplate
					...
					tmp_roi(ii).voxels = {ROI(ii).voxels{Dat.DataInd}(:,:,Dat.Slices(1),Dat.CurrentVol)};
				end
        tmp_roi(ii).fpath = {ROI(ii).fpath{Dat.DataInd}};
        tmp_roi(ii).fname = {ROI(ii).fname{Dat.DataInd}};
        tmp_roi(ii).label = ROI(ii).label;
        ...
        if length(tmp.ROI(1).voxels)==1 && ...
          ...
          if Dat.isDataMixed
            ...
            if ~isequal(size(tmp.ROI(1).voxels{1}),...
                size(DATA{Dat.DataInd}.FTDATA(:,:,Dat.Slices(1),Dat.CurrentVol)))
              fprintf(1,'Warning: Resizing ROIs to current data size...\n')
            end
            roi_slices = {};
            for ii=1:length(tmp.ROI)
            ...
            for kk=1:length(tmp.ROI)
              ...
              tmp.ROI(kk).voxels{1}(:,:,Dat.Slices(1),Dat.CurrentVol) = roi_slices{kk};
            end
            clear('roi_slices')
          end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function l_RoiComparisons(h,evd)
try
if length(ROI)<2
...
H.ROICOMPARISON_FIG = figure('units','pixel',...
                             'position',...
                             fig_pos,...
                             'Name','ROI Comparisons',...
                             'numbertitle','off',...
                             'Toolbar','none',...
                             'Menubar','none',...
                             'DoubleBuffer','on',... 
...
uipanel_h = uipanel('parent',H.ROICOMPARISON_FIG,...
                    'units','pixel',...
                    'position',[5 40 fig_w-10 fig_h-45]);
set(H.ROICOMPARISON_FIG,'color',get(uipanel_h,'backgroundcolor'))
% ROI A
...
cancel_btn = uicontrol('parent',H.ROICOMPARISON_FIG,...
                       'position',[tmp(1)+tmp(3)-65 5 65 30],...
                       'string','Cancel',...
                       'callback','delete(gcbf)');
tmp = get(cancel_btn,'position');
ok_btn = uicontrol('parent',H.ROICOMPARISON_FIG,...
                   'position',[tmp(1)-65-5 tmp(2) tmp(3) tmp(4)],...
                   'string','OK',...
                   'callback','uiresume(gcbf)');

% Wait for exit
uiwait(H.ROICOMPARISON_FIG)
if ishandle(H.ROICOMPARISON_FIG)
...
end % function l_RoiComparisons(h,
...
for ii=update_ind
  for kk=UpdatedRois
    ...
    if ii==1
      CurrentSlice = ROI(kk).voxels{Dat.DataInd}(:,:,Dat.Slices(1),Dat.CurrentVol);
    elseif ii==2
      CurrentSlice = reshape(ROI(kk).voxels{Dat.DataInd}(:,Dat.Slices(2),:,Dat.CurrentVol),...
                             Dat.ImageDim(Dat.DataInd,1),Dat.ImageDim(Dat.DataInd,3));
    else
      CurrentSlice = reshape(ROI(kk).voxels{Dat.DataInd}(Dat.Slices(3),:,:,Dat.CurrentVol),...
                             Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
    end
    
    tmp=find(CurrentSlice,1,'first');
    if isempty(tmp) || ~any(kk==Dat.RoiView)
      OldLines=findobj(H.ROIAX(ii,kk),'tag','roi_edgeline');
      if ~isempty(OldLines)
        delete(OldLines);
      end
      continue
...
if Dat.AxView==0
  ...
  for ii=update_ind
    for kk=roi_ind
      if ii==1
        tmp = ROI(kk).voxels{Dat.DataInd}(:,:,Dat.Slices(1),Dat.CurrentVol);
      elseif ii==2
        tmp = reshape(ROI(kk).voxels{Dat.DataInd}(:,Dat.Slices(2),:,Dat.CurrentVol),...
                      Dat.ImageDim(Dat.DataInd,1),Dat.ImageDim(Dat.DataInd,3));
      else
        tmp = reshape(ROI(kk).voxels{Dat.DataInd}(Dat.Slices(3),:,:,Dat.CurrentVol),...
                      Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
      end
...
% visible in this view...
  ...
  for kk=roi_ind
    ...
    if isempty(find(tmp,1,'first'))
      set(H.IMROI([1 2 3]~=Dat.AxView,kk),'visible','off')
      set(H.IMROI(Dat.AxView,kk),'visible','off')
      continue
    end
    tmp=uint8(tmp);
    ...
    set(H.IMROI([1 2 3]~=Dat.AxView,kk),'visible','off')
    set(H.IMROI(Dat.AxView,kk),'cdata',cdata,...
                      'AlphaDataMapping','none',...
                      'AlphaData',alphadata,...
                      'visible','on')
...
%% functions or slice/contrast viewing
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmpi(get(H.UITOGGLE_ARROW,'state'),'on') %% Slice/Contrast viewing
  if seltype==1
    % Check which image was pressed
    ind=find(h==H.IM);
    l_ChangeSlice([],[],'mouse',ind)
    set(H.FIG,'windowbuttonmotionfcn',...
              {@l_ChangeSlice,'mouse',ind},...
              'windowbuttonupfcn',...
              @l_WindowButtonUpFcn)
  ...
  elseif seltype==3
    ...
    % Use the mouse to set contrast and brightness
    cp = get(H.FIG,'currentpoint');
    Dat.tmp_point = cp;
    
    set(H.FIG,'windowbuttonmotionfcn',...
              {@l_SetContrastBrightnessGamma,[],1},...
              'windowbuttonupfcn',...
              @l_WindowButtonUpFcn)
  end
...
switch opt
 ...
 case 'add'
  ...
  if ax_ind==1
    Dat.RoiUndo(le+1).voxels = ROI(roi_ind).voxels{Dat.DataInd}(:,:,Dat.Slices(1),Dat.CurrentVol);
  ...
  elseif ax_ind==2
    ...
    Dat.RoiUndo(le+1).voxels = reshape(ROI(roi_ind).voxels{Dat.DataInd}(Dat.Slices(3),:,:,Dat.CurrentVol),...
                                       Dat.ImageDim(Dat.DataInd,2),Dat.ImageDim(Dat.DataInd,3));
    Dat.RoiUndo(le+1).ax_ind = ax_ind;
    Dat.RoiUndo(le+1).label = ROI(roi_ind).label;
...
if Dat.isDataMixed
	resp=questdlg({'Choose unfolding direction:','',...
		...
		'or',...
		['Up/Down (current Z-position = ',num2str(Dat.Slices(3)),')']},...
		'Choose direction',...
	...
	resp=questdlg({'Choose unfolding direction:','',...
		...
		['Up/Down (current Z-position = ',num2str(Dat.Slices(3)),')'],...
		'',...
		['Front/Back (current X-position = ',num2str(Dat.Slices(1)),')'],...
...
elseif strcmpi(resp,'Up/Down')
	if Dat.isDataMixed
		FoldInd = [-Dat.Slices(3) 0];
	else
		FoldInd = [-Dat.Slices(3) 0 0];
	end
elseif strcmpi(resp,'Front/Back')
	FoldInd = [0 0 -Dat.Slices(1)];
end
  ...
  function l_LaunchHeaderBrowser(fh,evd)
   %try
    if ( isfield(DATA{Dat.DataInd},'PROCPAR') && ~isempty(DATA{Dat.DataInd}.PROCPAR) ) || ...
        ( isfield(DATA{Dat.DataInd},'HDR') && ~isempty(DATA{Dat.DataInd}.HDR) )
...
function l_PrintLicense(h,evd)
  ...
  fprintf(1,'*\n')
  fprintf(1,'* Copyright (C) 2006 Juha-Pekka Niskanen <Juha-Pekka.Niskanen@uef.fi>\n');
  fprintf(1,'*\n')
