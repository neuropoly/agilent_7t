...
if Dat.isDataMixed
  Dat.XSize = length(DATA);
  Dat.YSize = 0;
  Dat.ZSize = 0;
else
  Dat.XSize = size(DATA{1}.FTDATA,3);
  Dat.YSize = size(DATA{1}.FTDATA,2);
  Dat.ZSize = size(DATA{1}.FTDATA,1);
...
function H=l_DrawGui
  ...
  rows = floor(sqrt(Dat.XSize));
  cols = ceil(Dat.XSize/rows);
  ...
  if Dat.isDataMixed
    ...
    set([H.YDIR_RADIO,H.ZDIR_RADIO],'enable','off')
  end
