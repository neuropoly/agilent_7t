function out = mri_dicom_convert(hdr,opts,root_dir,format)
   ...
   %xy1    = hdr{i}.ImagePositionPatient(:)*orient;
    ...
    for j=1:length(vol),
       ...
       %xy2    = vol{j}{1}.ImagePositionPatient(:)*orient;
        ...
        try
            %PP added line for AcquisitionNumber
...
for j=1:length(vol),
    ...
    max_slice = 0;
    max_rep = 0;
    %first find number of repetitions and of slices
    for i=1:length(vol{j}), 
        [dir0 fil0 ext0] = fileparts(vol{j}{i}.Filename);
        slice0 = strfind(fil0,'slice');
        image0 = strfind(fil0,'image');
        sliceN = str2double(fil0(slice0+5:slice0+7));
        repN = str2double(fil0(image0+5:image0+7));
        max_slice = max(sliceN,max_slice);
        max_rep = max(repN,max_rep);
    end;
