...
%
% This is a modifed version intended for use with the SPM adaptation
% of the Paxinos Atlas.
%
% Jesper Andersson 11/4-02
%
...
% Original version		def_brainmask = fullfile(SWD,'apriori','brainmask.img');
                                def_brainmask = fullfile(SWD,'ratlas/templates','brainmask.img'); % Paxinos version
...
% Original version fov = VF(1).dim(1:3).*sqrt(sum(VF(1).mat(1:3,1:3).^2));
tM = spm_matrix(prms');                                               % Paxinos version
fov = VF(1).dim(1:3).*sqrt(sum((tM(1:3,1:3)*VF(1).mat(1:3,1:3)).^2)); % Paxinos version
if any(fov<60),
