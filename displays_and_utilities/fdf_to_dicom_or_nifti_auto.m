...
erase_2d = 1;       % If set to 1, this will erase the 2d volumes to only keep the 3d and 4d files
...
for i_folder=1:nb_folders
    fdf_files=dir([protocol_path list(i_folder).name '/s*.fdf']);
    input={fdf_files(:).name};
    path = [protocol_path list(i_folder).name '/'];
...
%                     save_avw(data,output_file{k},'f',res);
                    aedes_write_nifti(data,output_file{k},'VoxelSize',res);
                catch exception
                    errordlg(['unable to write ' output_file{k}])
                end
