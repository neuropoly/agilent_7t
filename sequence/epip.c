...
pulsesequence()
{
  ...
  double t_x;
  double t_y;
  GENERIC_GRADIENT_T diffusion_x_half;
  GENERIC_GRADIENT_T diffusion_x;
  GENERIC_GRADIENT_T diffusion_x_y_half;
  GENERIC_GRADIENT_T diffusion_y;
...
if (tstartup) {
  ...
  if (DW) {
    ...
    t_x = tdelta/8*dro;
    t_y = tdelta-7*t_x;
    init_diffusion(&diffusion,&diff_grad,"diff",gdiff,tdelta); /* Diffusion */
    init_generic(&diffusion_x_half,"x_half",gdiff,t_x/2);
    init_generic(&diffusion_x,"x",gdiff,t_x);
    init_generic(&diffusion_y,"y",gdiff,t_y);
    init_generic(&diffusion_x_y_half,"x_y_half",gdiff,t_y/2+t_x/2);

    calc_generic(&diffusion_x_half,WRITE,"gdiff","t_x/2");
    calc_generic(&diffusion_x,WRITE,"gdiff","t_x");
    calc_generic(&diffusion_y,WRITE,"gdiff","t_y");
    calc_generic(&diffusion_x_y_half,WRITE,"gdiff","t_y/2+t_x/2");
  }
...
if (tstartup) {
  ...
  /**********************************************************/
		  ...
		  reste = del1-tdelta;
		  delay(reste/2);
		  obl_shapedgradient(diffusion_x_half.name,diffusion_x_half.duration,diffusion_x_half.amp,0,0,WAIT);
		  for (i = 1; i < 7; i++)  {
			obl_shapedgradient(diffusion_x.name,diffusion_x.duration,pow(-1,i)*diffusion_x.amp,0,0,WAIT);
		  }	  
		  obl_shapedgradient(diffusion_x_y_half.name,diffusion_x_y_half.duration,-diffusion_x_y_half.amp,0,0,WAIT);
		  obl_shapedgradient(diffusion_y.name,diffusion_y.duration,diffusion_y.amp,0,0,WAIT);
		  delay(reste/2);
            } 
            else delay(del1);
            obl_shapedgradient(crush_grad.name,crush_grad.duration,-crush_grad.amp,0.0,crush_grad.amp,WAIT);
  ...
  if (SGE) { 
    ...
    peloop('c',pe_steps,vpe_steps,vpe_ctr);
      ...
      sub(vpe_ctr,vssc,vpe_ctr);        /* vpe_ctr counts up from -ss1.00c */
