  ...
  peloop(seqcon[2],nseg,vseg,vseg_ctr);
    ...
    msloop(seqcon[1],ns,vms_slices,vms_ctr);
      ...
      if (spinecho[0] == 'y') {
        ...
        if (diff[0] == 'y') {
          double reste;
          reste = te_delay1-diffusion.grad->duration*10*16;
	  delay(reste/2);
	  double xmax_div2;
	  xmax_div2 = diffusion.grad->duration;
	  obl_shapedgradient(diffusion.grad->name,xmax_div2/dro,diffusion.grad->amp,diffusion.grad->amp,diffusion.grad->amp,WAIT);
	  for (i = 1; i < 7; i++)  {
		obl_shapedgradient(diffusion.grad->name,xmax_div2*2/dro,pow(-1,i)*diffusion.grad->amp,pow(-1,i)*diffusion.grad->amp,pow(-1,i)*diffusion.grad->amp,WAIT);
	  }
	  double durationy;
	  durationy = diffusion.grad->duration*10*16-14*xmax_div2/dro;	  
	  obl_shapedgradient(diffusion.grad->name,durationy/2+xmax_div2/dro,(-1)*diffusion.grad->amp,(-1)*diffusion.grad->amp,(-1)*diffusion.grad->amp,WAIT);
	  obl_shapedgradient(diffusion.grad->name,durationy/2,diffusion.grad->amp,diffusion.grad->amp,diffusion.grad->amp,WAIT);
	  delay(reste/2);
